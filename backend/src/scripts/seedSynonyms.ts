...
const GROUPS: { canonical: string; terms: { term: string; language: 'ar' | 'en' }[] }[] = [
...
async function main() {
  for (const g of GROUPS) {
    const existing = await pool.query(`SELECT id FROM synonym_groups WHERE canonical_term = $1`, [g.canonical]);
    let groupId: string;
    if (existing.rows.length > 0) {
      groupId = existing.rows[0].id;
    } else {
      const inserted = await pool.query(`INSERT INTO synonym_groups (canonical_term) VALUES ($1) RETURNING id`, [g.canonical]);
      groupId = inserted.rows[0].id;
    }
    for (const t of g.terms) {
      await pool.query(
        `INSERT INTO synonym_terms (synonym_group_id, term, language) VALUES ($1,$2,$3) ON CONFLICT (synonym_group_id, term) DO NOTHING`,
        [groupId, t.term, t.language],
      );
    }
  }
