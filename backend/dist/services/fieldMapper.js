...
exports.proposeMappingForHeaders = proposeMappingForHeaders;
exports.overlayPersistedMappings = overlayPersistedMappings;
exports.persistMapping = persistMapping;
exports.allColumnsConfident = allColumnsConfident;
exports.annotateConflicts = annotateConflicts;
const pool_1 = require("../db/pool");
const conceptDictionary_1 = require("./conceptDictionary");
const SEARCHABLE_BY_DEFAULT = [
...
const FILTERABLE_BY_DEFAULT = [
    'supplier_name', 'category_l1', 'category_l2', 'category_l3', 'manufacturer', 'country_of_origin', 'unit',
];
function defaultsFor(concept) {
    if (concept === 'internal_only')
        return { searchable: false, filterable: false, visibility: 'admin_only' };
    if (concept === 'unmapped')
        return { searchable: false, filterable: false, visibility: 'visible_user' };
    return {
 ...
 */
function proposeMappingForHeaders(headers) {
    ...
    return headers.map((header, columnIndex) => {
        const { concept, strong } = (0, conceptDictionary_1.proposeConceptForHeader)(header);
        const isDuplicateHeader = (nameCounts.get((header || '').trim()) ?? 0) > 1;
        const confidence = isDuplicateHeader ? 'auto_low' : strong ? 'auto_high' : concept === 'unmapped' ? 'auto_low' : 'auto_low';
 ...
 */
async function overlayPersistedMappings(agreementId, logicalSourceKey, proposals) {
    const { rows } = await pool_1.pool.query(`SELECT column_index, source_column_name, mapped_concept, is_searchable, is_filterable, visibility, confidence
     FROM field_mappings WHERE agreement_id = $1 AND logical_source_key = $2 AND confidence != 'auto_low'`, [agreementId, logicalSourceKey]);
    const persisted = new Map();
    for (const r of rows)
        persisted.set(`${r.column_index}::${r.source_column_name}`, r);
...
async function persistMapping(agreementId, logicalSourceKey, proposals, confirmedBy) {
    for (const p of proposals) {
        await pool_1.pool.query(`INSERT INTO field_mappings (agreement_id, logical_source_key, column_index, source_column_name,
       ...
       DO UPDATE SET mapped_concept = EXCLUDED.mapped_concept, is_searchable = EXCLUDED.is_searchable,
          ...
          updated_at = now()`, [agreementId, logicalSourceKey, p.columnIndex, p.sourceColumnName, p.mappedConcept, p.isSearchable, p.isFilterable, p.visibility, p.confidence, confirmedBy]);
    }
}
...
function allColumnsConfident(proposals) {
 ...
 */
function annotateConflicts(proposals) {
    const IGNORED = ['unmapped', 'internal_only'];
    const byConcept = new Map();
    for (const p of proposals) {
        if (IGNORED.includes(p.mappedConcept))
            continue;
    ...
    for (const [concept, cols] of byConcept) {
        if (cols.length > 1)
            conflicts.push({ concept, columns: cols });
    }
    const conflictMap = new Map();
    for (const c of conflicts)
        conflictMap.set(c.concept, c.columns.map((x) => x.columnIndex));
    const annotated = proposals.map((p) => {
        const columnsForConcept = conflictMap.get(p.mappedConcept);
        if (!columnsForConcept)
            return { ...p, hasConflict: false, conflictWith: [] };
