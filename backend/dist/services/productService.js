...
function rebuildRawKeys(headerColumns) {
    const seen = new Map();
...
async function getHiddenRawKeys(agreementId, logicalSourceKey, headerColumns) {
    const { rows } = await pool_1.pool.query(`SELECT column_index, source_column_name FROM field_mappings
     WHERE agreement_id = $1 AND logical_source_key = $2 AND visibility IN ('admin_only','hidden')`, [agreementId, logicalSourceKey]);
    if (rows.length === 0)
        return new Set();
 ...
 */
async function getProductDetail(productId, isAdmin) {
    const { rows } = await pool_1.pool.query(`SELECT p.*, a.slug AS agreement_slug, a.name_ar AS agreement_name_ar,
    ...
    if (rows.length === 0)
        return null;
    const row = rows[0];
    const hiddenKeys = isAdmin ? new Set() : await getHiddenRawKeys(row.agreement_id, row.logical_source_key, row.header_columns ?? []);
    const filterRaw = (raw) => {
    ...
    if (row.mapped_product_id) {
        ...
        const sib = await pool_1.pool.query(`SELECT p.* FROM products p
       JOIN sources s ON s.id = p.source_id
       WHERE p.agreement_id = $1 AND p.mapped_product_id = $2 AND p.id <> $3
         AND s.status = 'active' AND (s.is_visible_to_users = TRUE OR $4::boolean)`, [row.agreement_id, row.mapped_product_id, row.id, isAdmin]);
