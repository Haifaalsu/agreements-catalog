...
exports.expandQueryWords = expandQueryWords;
exports.search = search;
exports.computeFacets = computeFacets;
const pool_1 = require("../db/pool");
const textNormalize_1 = require("./textNormalize");
let synonymCache = null;
const SYNONYM_CACHE_TTL_MS = 60000;
/** Loads (and briefly caches) the synonym dictionary, normalized, as term -> [all sibling terms incl. itself]. */
async function loadSynonymMap() {
    if (synonymCache && Date.now() - synonymCache.loadedAt < SYNONYM_CACHE_TTL_MS) {
        return synonymCache.byTerm;
    }
    const { rows } = await pool_1.pool.query(`SELECT synonym_group_id AS group_id, term FROM synonym_terms`);
    const byGroup = new Map();
    for (const r of rows) {
        const norm = (0, textNormalize_1.normalizeText)(r.term);
        if (!norm)
            continue;
...
async function expandQueryWords(query) {
    const normalized = (0, textNormalize_1.normalizeText)(query);
...
async function search(params) {
    ...
    const whereClauses = [`a.display_type = 'standard'`, `s.status = 'active'`, `s.is_visible_to_users = TRUE`];
    const values = [];
    let scoreExpr = '0';
    const push = (v) => {
        values.push(v);
    ...
    for (const alternatives of wordGroups) {
        ...
        whereClauses.push(`(search_norm_product_id ILIKE ANY(${partialPh}) OR search_norm_identity ILIKE ANY(${partialPh}) OR
    ...
    if (f.agreementId)
        whereClauses.push(`a.id = ${push(f.agreementId)}`);
    if (f.categoryL1)
        whereClauses.push(`p.mapped_category_l1 = ${push(f.categoryL1)}`);
    if (f.categoryL2)
        whereClauses.push(`p.mapped_category_l2 = ${push(f.categoryL2)}`);
    if (f.categoryL3)
        whereClauses.push(`p.mapped_category_l3 = ${push(f.categoryL3)}`);
    if (f.supplierName)
        whereClauses.push(`p.mapped_supplier_name = ${push(f.supplierName)}`);
    if (f.manufacturer)
        whereClauses.push(`p.mapped_manufacturer = ${push(f.manufacturer)}`);
    if (f.countryOfOrigin)
        whereClauses.push(`p.mapped_country_of_origin = ${push(f.countryOfOrigin)}`);
    if (f.unit)
        whereClauses.push(`p.mapped_unit = ${push(f.unit)}`);
    if (f.sourceFileName)
        whereClauses.push(`s.original_file_name = ${push(f.sourceFileName)}`);
    ...
    const orderSql = params.sort === 'name' ? `best.mapped_name_ar NULLS LAST, best.mapped_description_ar NULLS LAST` :
 ...
 */
async function computeFacets(params) {
    const wordGroups = params.query.trim() ? await expandQueryWords(params.query) : [];
    const whereClauses = [`a.display_type = 'standard'`, `s.status = 'active'`, `s.is_visible_to_users = TRUE`];
    const values = [];
    const push = (v) => {
    ...
    async function facet(col, extra = '') {
        const { rows } = await pool_1.pool.query(`SELECT ${col} AS value, count(*)::int AS count ${baseFrom} AND ${col} IS NOT NULL ${extra} GROUP BY ${col} ORDER BY count DESC LIMIT 50`, values);
        return rows;
    }
    const [agreementsRaw, categoriesL1, suppliers, manufacturers, countries, units] = await Promise.all([
        pool_1.pool.query(`SELECT a.id AS value, a.slug, a.name_ar, count(*)::int AS count ${baseFrom} GROUP BY a.id, a.slug, a.name_ar ORDER BY count DESC`, values),
