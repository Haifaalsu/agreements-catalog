 ...
 */
function detectHeaderRow(sampleRows) {
    ...
    for (let i = 0; i <= maxCandidate; i++) {
        ...
        if (nextLooksLikeData)
            score += 8;
        if (nextNextFilled >= Math.max(2, filled * 0.4))
            score += 4;
        if (isLikelyTitleOrNote)
            score -= 15;
        // Header rows rarely repeat the exact same text as the row below them.
        if (nextRow && JSON.stringify(row) === JSON.stringify(nextRow))
            score -= 20;
    ...
    const confidence = best && best.score >= 15 && (!second || best.score - second.score >= 6) ? 'high' : 'low';
